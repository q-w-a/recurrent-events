...
# no treatment
d_no_treat <- function(x,y) {
  return(tibble(A1=rep(0, length(x)),
                A2 = rep(0, length(x))))
}

# 1 wolf is killed, no delay 
d_dose_1_no_delay <- function(x,y) {
  return(tibble(A1=rep(1, length(x)),
                A2 = rep(0, length(x))))
}

# 1 wolf is killed, with delay 
...
mean_no_treat <- get_counterfactual_mean(
  years = N_YEARS,
  n_packs = N_PACKS,
  intervention = d_no_treat)
  
mean_treat_no_delay <- get_counterfactual_mean(
  years = N_YEARS,
  n_packs = N_PACKS,
  intervention = d_dose_1_no_delay)

mean_treat_delay <- get_counterfactual_mean(
  years = N_YEARS,
  n_packs = N_PACKS,
  intervention = d_dose_1_delay)

mean_shift_treatment_up <- get_counterfactual_mean(
  years = N_YEARS,
  n_packs = N_PACKS,
  intervention = d_shift_dose_up)
