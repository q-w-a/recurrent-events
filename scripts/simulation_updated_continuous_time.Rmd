...
NEED TO INCORPORATE RIGHT TRUNCATION 
...
get_outcome_at_k <- function(time_point, dat, n_packs, interval_length, lambda_0=10) {
    ...
    A2_effect <- case_when(A2_prev == "No treatment" ~ 0,
                           A2_prev == "Delay <= 14" ~ 5,
                           A2_prev == "14 < Delay <= 30" ~ 2,
                           A2_prev == "Delay > 30" ~ 1)
    ...
    prob_event <- plogis(
      ...
      -12 + 
        ...
        # stronger effect if delay is small 
        - 5 * A1_prev*A2_effect
        - 5 * W_prev +
        # less likely if take out breeding female
        - 10 * A3_prev 
       )*(plogis(interval_length)^8)
    
     # cat(paste(signif(range(prob_event),3)), "\n")
...
get_treatment_at_k <- function(time_point, dat, n_packs,
  ...
  if(!poach_harvest_only){
    prob_treat <- plogis(-6.5 + 2*Y_prev + .7*(X_prev)/sqrt(lambda_0) 
                         - .5* W_prev+
                           rnorm(n_packs,0,3))*(plogis(interval_length)^(7))
  } else{
      prob_treat <- plogis(-6.5 +.7*(X_prev)/sqrt(lambda_0) 
                           - .5* W_prev+ 
                             rnorm(n_packs,0,3))*(plogis(interval_length)^(7))
  }
  ...
  delay <- recent_event %>%
    mutate(delay = ifelse(prev_event,
                          runif(n_packs, min = (most_recent-1)*interval_length, 
                         max=most_recent*interval_length),
                         runif(n_packs, 
                                min = interval_length*time_point,
                                max = interval_length*time_point*2)),
           delay = case_when(
             delay*7 < 14 ~ "Delay <= 14",
  ...
  if (!is.null(intervention)) {
    assigned <- intervention(A11, A12, A13, Y_prev)
...
d_kill_wolf_in_response <- function(x,y,z,w) {
